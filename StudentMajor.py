...
class StudentMajor(Base):
    ...
    majors: Mapped["Major"] = relationship(back_populates="students")
    majorName: Mapped[str] = mapped_column('major_name', ForeignKey("majors.name"), primary_key=True)
    students: Mapped["Student"] = relationship(back_populates="majors")
    ...
    def __init__(self, students, majors, declaration_date: datetime):
        self.students = students
        self.majors = majors
        self.studentId = students.studentID
        self.majorName = majors.name
